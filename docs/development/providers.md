...
`ProviderOptions.RefreshInterval` (default 60 s). Before entering the loop, `AssignProviderIds()`
computes a `ProviderId` for each `ServicesProviderConfig` that doesn't already have one set from
configuration. The id is `{hostname}-{type}` with an index suffix when multiple configs share the
same base id. On each tick:

1. Read [`ProviderOptions.ServicesProviders`](../../src/Dsm.Providers/Options/ProviderOptions.cs) — a
   list of typed configs, each carrying the provider's own settings and `ProviderId`.
2. For each entry, resolve an `IServicesProvider` from the factory and call `ListServices()`.
   Each provider stamps `_config.ProviderId` on the `Service` objects it creates.
...
| `Hostname` | no | Override for the hostname component of auto-derived ProviderIds. Falls back to `HOSTNAME` env var, then `Environment.MachineName` |
...
| `ProviderId` | no | Override for the auto-derived provider identity. When set, used as-is instead of `{hostname}-{type}[-{index}]` |
...
`Service` (the wire contract) carries three fields the provider can populate that aren't already in
the table above:
...
- `ProviderId` — identifies which provider source contributed the service. Auto-derived by
  `ProviderService.AssignProviderIds()` as `{hostname}-{type}[-{index}]` and stamped onto each
  service by the individual provider implementations via `_config.ProviderId`. The manager-side
  `ServicesCombiner` uses this to scope replacement: a POST only replaces services from the same
  provider, not services from other providers for the same hostname. Persisted as `_providerId`
  in Homepage YAML and as a `provider=` tag in Dashy.
...
1. Implement [`IServicesProvider`](../../src/Dsm.Providers/ServicesProviders/IServicesProvider.cs). The
   ...
   [`ContainerLabelServiceFactory`](../../src/Dsm.Providers/Services/ContainerLabelServiceFactory.cs)
   if your source exposes labels; otherwise build `Service` objects directly. Set
   `providerId: _config.ProviderId` when constructing `Service` objects so the manager-side
   combiner can scope replacement correctly.
2. Add a variant to
   [`ServicesProviderType`](../../src/Dsm.Providers/Options/ServicesProviderType.cs).
