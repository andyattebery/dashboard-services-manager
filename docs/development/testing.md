...
The two projects currently differ — Providers tests use terse `snake_case` (`ExtractFirstHost_returns_first_host`), Managers tests use `PascalCase_With_Underscores` (`NativePrefix_PassesThroughWithoutTranslation`). Both forms are acceptable NUnit conventions; we haven't unified yet. **If you're adding tests to an existing fixture, follow that fixture's local style.** A repo-wide rename pass is tracked separately and is not a precondition for new work.
