...
public class HomepageDashboardManager : IDashboardManager
{
    ...
    private static Dictionary<string, HomepageServiceEntry> ToEntryMap(
        ...
        IReadOnlyDictionary<string, HomepageServiceWidgetEntry> widgets,
        ILogger logger)
    {
        ...
        if (!widgets.TryGetValue(service.Name, out var w) &&
            !string.IsNullOrEmpty(service.ServiceDefaultsName))
        {
            widgets.TryGetValue(service.ServiceDefaultsName, out w);
        }
        if (w is not null)
        {
    ...
    private async Task<Dictionary<string, HomepageServiceWidgetEntry>> LoadWidgetEntries()
    {
        var result = new Dictionary<string, HomepageServiceWidgetEntry>(StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var map in entries)
        {
            ...
            foreach (var (name, entry) in map)
            {
                ...
                {
                    _logger.LogWarning(
                        "Homepage service widgets file has duplicate entry for service '{Name}'; last-wins.",
                        name);
                }
                result[name] = entry;
            }
